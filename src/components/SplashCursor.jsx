...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl, internalFormat, format, type) {
      if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        if ("drawBuffers" in gl) { // essentially isWebGL2
    ...
    function getUniforms(program) {
      ...
      for (let i = 0; i < uniformCount; i++) {
        const uniformName = gl.getActiveUniform(program, i);
        if (uniformName) {
          uniforms[uniformName.name] = gl.getUniformLocation(program, uniformName.name);
        }
      }
      return uniforms;
    }
    ...
    function splat(x, y, dx, dy, color) {
      ...
      if (splatProgram.uniforms.radius) gl.uniform1f(splatProgram.uniforms.radius, correctRadius(config.SPLAT_RADIUS / 100.0));
      blit(velocity.write);
      velocity.swap();
    ...
    function generateColor() {
      let c = HSVtoRGB(0.05 + Math.random() * 0.07, 1.0, 1.0);
      c.r *= 0.15;
      c.g *= 0.15;
      c.b *= 0.15;
      return c;
    ...
    const onMouseDown = (e) => {
      ...
      splat(pointer.texcoordX, pointer.texcoordY, 10 * (Math.random() - 0.5), 30 * (Math.random() - 0.5), generateColor());
    };
    ...
    const onTouchStart = (e) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        if (posX >= rect.left && posX <= rect.right && posY >= rect.top && posY <= rect.bottom) {
          let scaleX = scaleByPixelRatio(posX - rect.left);
          let scaleY = scaleByPixelRatio(posY - rect.top);
          update(); // start loop
          updatePointerDownData(pointer, touches[i].identifier, scaleX, scaleY);
    ...
    const onTouchMove = (e) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        if (posX >= rect.left && posX <= rect.right && posY >= rect.top && posY <= rect.bottom) {
          let scaleX = scaleByPixelRatio(posX - rect.left);
          let scaleY = scaleByPixelRatio(posY - rect.top);
          updatePointerMoveData(pointer, scaleX, scaleY, pointer.color);
        }
      }
    };
    ...
    window.addEventListener("touchstart", onTouchMoveFirst, { passive: false });
    ...
    return () => {
      ...
      window.removeEventListener("touchstart", onTouchMoveFirst);
  ...
  return isSupported ? (
    <div style={{ position: "absolute", top: 0, left: 0, zIndex: 10, pointerEvents: "none", width: "100%", height: "100%" }}>
